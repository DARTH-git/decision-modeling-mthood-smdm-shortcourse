...
ru.S1S2 <- 0.03  # decrease in utility of treated sick individuals with every 
                 # additional year being sick/sicker
v.x <- runif(n.i, 0.95, 1.05) # vector capturing individuals' effect modifier at baseline

##################################### Functions ###########################################

# THE NEW samplev() FUNCTION
# efficient implementation of the vectorized categorical distribution #### 
...
MicroSim <- function(v.M_1, n.i, n.t, v.n, X = NULL, d.c, d.e, TR.out = TRUE, TS.out = TRUE, Trt = FALSE, seed = 1) {
  ...
  for (t in 1:n.t) { # t <- 3
    ...
    m.M[, t + 1] <- samplev(m_Probs = m.p)   # sample the next health state and store that state in matrix m.M 
...
Effs <- function(M_it, dur, Trt = FALSE, cl = 1, X = NULL) {
  ...
  u.it[M_it == "S1"] <- X[M_it == "S1"] * Trt * (u.Trt - dur[M_it == "S1"] * ru.S1S2) + (1 - Trt) * u.S1 # update the utility if sick conditional on treatment and duration of being sick/sicker
